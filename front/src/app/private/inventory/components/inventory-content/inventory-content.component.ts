...
export class InventoryContentComponent implements OnInit {
  ...
  private setupCategoryFilter(): void {
    this.searchForm.get('categoryFilter')?.valueChanges
      .pipe(
        debounceTime(300),
        distinctUntilChanged()
      )
  ...
  private filterInventoryLines(): void {
    ...
    this.filteredInventoryLines = this.tInventoryLine.filter(line => {
      ...
      const matchesCategory = !categoryFilter || line.oArticle.oCategory.id === categoryFilter;
  ...
  private getValueForSort(line: any, column: string): any {
    switch (column) {
      case 'referenceCode': return line.oArticle.referenceCode;
      case 'label': return line.oArticle.label;
      case 'category': return line.oArticle.oCategory.label;
      case 'quantity': return line.quantity;
      case 'value': return line.oArticle.unitPrice * line.quantity;
      default: return '';
    }
  }
